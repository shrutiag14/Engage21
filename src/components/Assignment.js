...
// 		database.collection('users').get().then((user) => {
// 			let studentID = '';
...
// 		database.collection('subjects').get().then((assignments) => {
// 			let currentIndex = 0;
// 			assignments.forEach((assignment) => {
// 				if (assignmentIndex === currentIndex) {
// 					this.setState({
// 						assignmentNumber: assignment.data().assignmentNumber,
// 						assignmentDescription: assignment.data().assignmentDescription,
// 						assignmentFiles: assignment.data().assignmentFiles || [],
// 						submissionDate: assignment.data().submissionDate
// 					});
// 					assignmentNumber = assignment.data().assignmentNumber;
// 					submissionDate = assignment.data().submissionDate;
// 				}
...
// 			if (userType === 'Student') {
// 				database.collection('users/' + this.props.dbUserKey + '/userAssignments/' + subjectCode + '_' + subjectName + '/assignment_' + assignmentNumber + '/assignmentFiles').on('value', (assignmentFiles) => {
// 					let uploadedFiles = [];
// 					assignmentFiles.forEach((assignmentFile) => {
// 						uploadedFiles.push({ ...assignmentFile.data() });
// 					});
...
// 				database.collection('users/' + this.props.dbUserKey + '/userAssignments/' + subjectCode + '_' + subjectName + '/assignment_' + assignmentNumber).on('value', (assignment) => {
// 					if (!assignment.data().isDone && moment().isSameOrBefore(submissionDate))
// 						submissionStatus = 'Pending';
// 					if (!assignment.data().isDone && !moment().isSameOrBefore(submissionDate))
// 						submissionStatus = 'Missing';
// 					if (assignment.data().isDone && !moment(assignment.val().submittedOn).isSameOrBefore(submissionDate))
// 						submissionStatus = 'Done Late';
// 					if (assignment.data().isDone && moment(assignment.val().submittedOn).isSameOrBefore(submissionDate))
// 						submissionStatus = 'Submitted';

// 					this.setState({
// 						submittedOn: assignment.data().submittedOn,
// 						submissionStatus
// 					});
// 				});
// 			}
...
// 				database.collection('users').on('value', (users) => {
// 					users.forEach((user) => {
// 						if (user.data().userType === 'Student' && user.child('/userAssignments/' + currentSubject).exists()) {
// 							database.ref('users/' + user.key + '/userAssignments/' + currentSubject + '/assignment_' + assignmentNumber).on('value', (assignment) => {
// 								if (assignment.data().isDone) {
// 									let uploadedFiles = [];
// 									database.collection('users/' + user.key + '/userAssignments/' + currentSubject + '/assignment_' + assignmentNumber + '/assignmentFiles').on('value', (assignmentFiles) => {
// 										assignmentFiles.forEach((assignmentFile) => {
