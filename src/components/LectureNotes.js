...
class LectureNotes extends React.Component {
	...
	componentDidMount() {
		...
		database.collection('subjects').get().then((subjects) => {
			subjects.forEach((subject) => {
				if (subject.data().subjectCode === subjectCode && subject.data().subjectName === subjectName) {
					database.collection('subjects').get().then((lectureNotes) => {
						lectureNotes.forEach((lectureNote) => {
							files.push({ ...lectureNote.data() });
						});
						this.setState({ files });
						files = [];
					});
	...
	uploadFile() {
		...
		storage.re('lecture_notes/' + this.props.subjectCode + '_' + this.props.subjectName + '/' + file.name).put(file).on('state_changed',
			(fileSnapshot) => {
			...
			() => {
				...
				database.collection('subjects').get().then((subjects) => {
					subjects.forEach((subject) => {
						if (subject.data().subjectCode === subjectCode && subject.data().subjectName === subjectName) {
							database.collection('subjects').push({
								filePath: path,
								fileName: file.name
							});
						}
