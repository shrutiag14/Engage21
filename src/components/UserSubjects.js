...
class UserSubjects extends React.Component {
	...
	componentDidMount() {
		let userSubjectKeys = [];
		let userSubjects = [];
		database.collection('users/' + this.props.dbUserKey + '/userSubjects').get()
		.then((subjects)=>{
			...
			userSubjectKeys.forEach((key) => {
				database.collection('subjects/' + key).get()
				.then((subject) => {
					userSubjects.push({
	...
	// 	let userSubjectKeys = [];
	// 	let userSubjects = [];
	// 	database.collection('users/' + this.props.dbUserKey + '/userSubjects').on('value', (subjects) => {
	// 		subjects.forEach((subject) => {
				
	// 			userSubjectKeys.push(subject.val().dbSubjectKey);
	// 		});
	// 		userSubjectKeys.forEach((key) => {
	// 			database.collection('subjects/' + key).once('value').then((subject) => {
	// 				userSubjects.push({
	// 					subjectName: subject.val().subjectName,
	// 					subjectCode: subject.val().subjectCode,
	// 					instructorName: subject.val().instructorName
